...
public class GamePanel extends JPanel {
    ...
    private void loadChomperImages() {
        chomperReady = Chomper.getCardReady();
        chomperFaded = Chomper.getCardFaded();
        chomperPreview = Chomper.getDragImageStatic();
    }
    @Override
    protected void paintComponent(Graphics g) {
        ...
        g.setColor(Color.BLACK);
        g.drawString("150", 570, 95);

        // === CARD 8: CHERRY BOMB (x=650) ===
        if (game.sun >= 150 && cherryBombReady != null) {
            g.drawImage(cherryBombReady, 650, 10, , 70, null);
        ...
        // 3. Draw Active Flying Bullets over the Zombies!
    ...
    public void handleClick(int x, int y) {
        ...
        // Xử lý click trên thanh card (chọn plant)
        if (y <= 100) {
            if (x >= 110 && x <= 170) {
